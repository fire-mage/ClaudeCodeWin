...
public class PlannerService
{
    ...
    private void WireEvents(PlannerSession session)
    {
        ...
        cli.OnError += error =>
        {
            // Just remove from tracking — CLI already errored out, no need to StopSession
            _sessions.TryRemove(featureId, out _);
    ...
    /// <summary>
    /// Extract a JSON object from text. Handles ```json fenced blocks and raw JSON.
    /// </summary>
    private static string? ExtractJsonBlock(string text)
    {
        ...
        // Try to find a raw JSON object with "title" and "phases"
            ...
            for (var j = i; j < text.Length; j++)
            {
                ...
                else if (ch == '}')
                {
                    ...
                    if (depth == 0)
                    {
                        ...
                        if (candidate.Contains("\"title\"") && candidate.Contains("\"phases\"")
