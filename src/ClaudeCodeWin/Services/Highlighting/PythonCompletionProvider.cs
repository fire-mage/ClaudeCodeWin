...
public class PythonCompletionProvider : ICompletionProvider
{
    ...
    public List<CompletionItem> GetCompletions(string text, int caretPosition, CompletionTrigger trigger, List<SyntaxToken> tokens)
    {
        ...
        // Snippets
        items.AddRange(PythonSnippets.All);
        ...
        while (i < len)
        {
            ...
            if (char.IsLetter(c) || c == '_')
            {
                ...
                if (i - start >= 2)
                {
                    ...
                    if (!PythonTokenizer.Keywords.Contains(word) &&
                        !PythonTokenizer.ControlKeywords.Contains(word) &&
                        !PythonTokenizer.TypeKeywords.Contains(word) &&
                        !PythonTokenizer.LiteralKeywords.Contains(word) &&
                        !PythonTokenizer.BuiltinFunctions.Contains(word) &&
                        seen.Add(word))
                    {
                        result.Add(new CompletionItem
                        {
    ...
    private static List<CompletionItem> BuildKeywordItems()
    {
        ...
        foreach (var kw in allKeywords)
        {
            ...
            var kind = PythonTokenizer.TypeKeywords.Contains(kw)
    ...
    {
        var items = new List<CompletionItem>();
        foreach (var fn in PythonTokenizer.BuiltinFunctions)
        {
