...
public class RustCompletionProvider : ICompletionProvider
{
    private static readonly HashSet<string> s_snippetLabels =
        new(RustSnippets.All.Select(s => s.Label), StringComparer.Ordinal);

    private static readonly List<CompletionItem> s_keywordItems = BuildKeywordItems();

    public CompletionTrigger? GetTriggerForCharacter(char c) => c switch
    {
    ...
    public List<CompletionItem> GetCompletions(string text, int caretPosition, CompletionTrigger trigger, List<SyntaxToken> tokens)
    {
        ...
        // Snippets
        items.AddRange(RustSnippets.All);
        ...
        if (prefix.Length > 0)
        {
            // Strip trailing '!' from prefix for matching against non-macro items
            string matchPrefix = prefix.TrimEnd('!');
            items = items.Where(item =>
                item.Label.StartsWith(matchPrefix, StringComparison.OrdinalIgnoreCase) ||
        ...
        while (i < len)
        {
            ...
            if (tokenIdx < tokens.Count)
            {
                ...
                if (i >= tok.Start && i < tok.Start + tok.Length)
                {
                    ...
                    if (tok.Type == SyntaxTokenType.TypeName)
                    {
                        ...
                        if (word.Length >= 2 && !RustTokenizer.MacroNames.Contains(word.TrimEnd('!')) && seen.Add(word))
                        {
            ...
            if (char.IsLetter(c) || c == '_')
            {
                ...
                if (i - start >= 2)
                {
                    ...
                    if (!RustTokenizer.Keywords.Contains(word) &&
                        !RustTokenizer.ControlKeywords.Contains(word) &&
                        !RustTokenizer.TypeKeywords.Contains(word) &&
                        !RustTokenizer.LiteralKeywords.Contains(word) &&
                        seen.Add(word))
                    {
                        result.Add(new CompletionItem
                        {
    ...
    private static List<CompletionItem> BuildKeywordItems()
    {
        ...
        // All keywords
        var allKeywords = new HashSet<string>(RustTokenizer.Keywords);
        allKeywords.UnionWith(RustTokenizer.ControlKeywords);
        allKeywords.UnionWith(RustTokenizer.LiteralKeywords);
        ...
        // Type keywords
        foreach (var tp in RustTokenizer.TypeKeywords)
        {
            if (s_snippetLabels.Contains(tp))
                continue;
            ...
            items.Add(new CompletionItem
            {
                Label = tp,
                InsertText = tp,
                Kind = CompletionItemKind.TypeKeyword,
                SortPriority = 2,
            });
        }

        // Common macros
        foreach (var macro in RustTokenizer.MacroNames)
        {
            string macroWithBang = macro + "!";
            if (s_snippetLabels.Contains(macroWithBang))
                continue;

            items.Add(new CompletionItem
            {
                Label = macroWithBang,
                InsertText = macroWithBang,
                Kind = CompletionItemKind.Keyword,
                Detail = "macro",
                SortPriority = 2,
            });
