...
public class GoCompletionProvider : ICompletionProvider
{
    private static readonly HashSet<string> s_snippetLabels =
        new(GoSnippets.All.Select(s => s.Label), StringComparer.Ordinal);
    ...
    private static List<CompletionItem> ExtractIdentifiers(string text, List<SyntaxToken> tokens)
    {
        ...
        while (i < len)
        {
            ...
            if (tokenIdx < tokens.Count)
            {
                ...
                {
                    if (tok.Type is SyntaxTokenType.String or SyntaxTokenType.Comment)
                    {
                    ...
                    if (tok.Type == SyntaxTokenType.TypeName)
                    {
                        ...
                        // Skip built-in functions — they are already in keyword items
                        if (word.Length >= 2 && !GoTokenizer.BuiltinFunctions.Contains(word) && seen.Add(word))
                        {
            ...
            if (char.IsLetter(c) || c == '_')
            {
                ...
                if (i - start >= 2)
                {
                    ...
                    if (!GoTokenizer.Keywords.Contains(word) &&
                        !GoTokenizer.ControlKeywords.Contains(word) &&
                        !GoTokenizer.TypeKeywords.Contains(word) &&
                        !GoTokenizer.LiteralKeywords.Contains(word) &&
                        !GoTokenizer.BuiltinFunctions.Contains(word) &&
                        seen.Add(word))
                    {
                        result.Add(new CompletionItem
                        {
    ...
    {
        var allKeywords = new HashSet<string>(GoTokenizer.Keywords);
        allKeywords.UnionWith(GoTokenizer.ControlKeywords);
        allKeywords.UnionWith(GoTokenizer.TypeKeywords);
        allKeywords.UnionWith(GoTokenizer.LiteralKeywords);
        ...
        foreach (var kw in allKeywords)
        {
            ...
            var kind = GoTokenizer.TypeKeywords.Contains(kw)
        ...
        // Built-in functions (shown with TypeKeyword kind to match highlighting)
        foreach (var fn in GoTokenizer.BuiltinFunctions)
        {
