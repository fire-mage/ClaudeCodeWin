...
public class JavaCompletionProvider : ICompletionProvider
{
    private static readonly HashSet<string> s_snippetLabels =
        new(JavaSnippets.All.Select(s => s.Label), StringComparer.Ordinal);
    ...
    public List<CompletionItem> GetCompletions(string text, int caretPosition, CompletionTrigger trigger, List<SyntaxToken> tokens)
    {
        ...
        // Snippets
        items.AddRange(JavaSnippets.All);
        ...
        while (i < len)
        {
            ...
            if (tokenIdx < tokens.Count)
            {
                ...
                if (i >= tok.Start && i < tok.Start + tok.Length)
                {
                    ...
                    if (tok.Type is SyntaxTokenType.Keyword or SyntaxTokenType.ControlKeyword
                        ...
                        or SyntaxTokenType.Preprocessor)
                    {
                        i = tok.Start + tok.Length;
                        continue;
                    }
            ...
            if (char.IsLetter(c) || c == '_' || c == '$')
            {
                ...
                if (i - start >= 2)
                {
                    ...
                    if (!JavaTokenizer.Keywords.Contains(word) &&
                        !JavaTokenizer.ControlKeywords.Contains(word) &&
                        !JavaTokenizer.TypeKeywords.Contains(word) &&
                        !JavaTokenizer.LiteralKeywords.Contains(word) &&
                        seen.Add(word))
                    {
                        result.Add(new CompletionItem
                        {
    ...
    {
        var allKeywords = new HashSet<string>(JavaTokenizer.Keywords);
        allKeywords.UnionWith(JavaTokenizer.ControlKeywords);
        allKeywords.UnionWith(JavaTokenizer.TypeKeywords);
        allKeywords.UnionWith(JavaTokenizer.LiteralKeywords);
        ...
        foreach (var kw in allKeywords)
        {
            ...
            var kind = JavaTokenizer.TypeKeywords.Contains(kw)
