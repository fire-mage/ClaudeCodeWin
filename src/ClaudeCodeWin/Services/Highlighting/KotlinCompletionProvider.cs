...
public class KotlinCompletionProvider : ICompletionProvider
{
    private static readonly HashSet<string> s_snippetLabels =
        new(KotlinSnippets.All.Select(s => s.Label), StringComparer.Ordinal);
    ...
    public List<CompletionItem> GetCompletions(string text, int caretPosition, CompletionTrigger trigger, List<SyntaxToken> tokens)
    {
        ...
        // Snippets
        items.AddRange(KotlinSnippets.All);
        ...
        while (i < len)
        {
            ...
            if (char.IsLetter(c) || c == '_')
            {
                ...
                if (i - start >= 2)
                {
                    ...
                    if (!KotlinTokenizer.Keywords.Contains(word) &&
                        !KotlinTokenizer.ControlKeywords.Contains(word) &&
                        !KotlinTokenizer.TypeKeywords.Contains(word) &&
                        !KotlinTokenizer.LiteralKeywords.Contains(word) &&
                        seen.Add(word))
                    {
                        result.Add(new CompletionItem
                        {
    ...
    private static List<CompletionItem> BuildKeywordItems()
    {
        ...
        // All keywords
        var allKeywords = new HashSet<string>(KotlinTokenizer.Keywords);
        allKeywords.UnionWith(KotlinTokenizer.ControlKeywords);
        allKeywords.UnionWith(KotlinTokenizer.LiteralKeywords);
        ...
        // Type keywords
        foreach (var tp in KotlinTokenizer.TypeKeywords)
        {
