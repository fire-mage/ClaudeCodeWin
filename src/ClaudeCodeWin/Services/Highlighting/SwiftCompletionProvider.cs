...
public class SwiftCompletionProvider : ICompletionProvider
{
    private static readonly HashSet<string> s_snippetLabels =
        new(SwiftSnippets.All.Select(s => s.Label), StringComparer.Ordinal);
    ...
    public List<CompletionItem> GetCompletions(string text, int caretPosition, CompletionTrigger trigger, List<SyntaxToken> tokens)
    {
        ...
        // Snippets
        items.AddRange(SwiftSnippets.All);
        ...
        while (i < len)
        {
            ...
            if (char.IsLetter(c) || c == '_')
            {
                ...
                if (i - start >= 2)
                {
                    ...
                    if (!SwiftTokenizer.Keywords.Contains(word) &&
                        !SwiftTokenizer.ControlKeywords.Contains(word) &&
                        !SwiftTokenizer.TypeKeywords.Contains(word) &&
                        !SwiftTokenizer.LiteralKeywords.Contains(word) &&
                        seen.Add(word))
                    {
                        result.Add(new CompletionItem
                        {
    ...
    private static List<CompletionItem> BuildKeywordItems()
    {
        ...
        // All keywords
        var allKeywords = new HashSet<string>(SwiftTokenizer.Keywords);
        allKeywords.UnionWith(SwiftTokenizer.ControlKeywords);
        allKeywords.UnionWith(SwiftTokenizer.LiteralKeywords);
        ...
        // Type keywords
        foreach (var tp in SwiftTokenizer.TypeKeywords)
        {
