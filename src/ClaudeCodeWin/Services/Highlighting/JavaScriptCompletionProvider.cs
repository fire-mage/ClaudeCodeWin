...
public class JavaScriptCompletionProvider : ICompletionProvider
{
    private static readonly HashSet<string> s_snippetLabels =
        new(JavaScriptSnippets.All.Select(s => s.Label), StringComparer.Ordinal);
    ...
    public List<CompletionItem> GetCompletions(string text, int caretPosition, CompletionTrigger trigger, List<SyntaxToken> tokens)
    {
        ...
        if (trigger == CompletionTrigger.Angle)
        {
            // After '<': show PascalCase identifiers (JSX components)
            var identifiers = ExtractIdentifiers(text, tokens)
                .Where(item => item.Label.Length >= 2 && char.IsUpper(item.Label[0]))
                .ToList();
            identifiers.Sort((a, b) => string.Compare(a.Label, b.Label, StringComparison.OrdinalIgnoreCase));
            return identifiers.Count > 50 ? identifiers.GetRange(0, 50) : identifiers;
        }
        ...
        // Snippets
        items.AddRange(JavaScriptSnippets.All);
        ...
        while (i < len)
        {
            ...
            if (char.IsLetter(c) || c == '_' || c == '$')
            {
                ...
                if (i - start >= 2)
                {
                    ...
                    if (!JavaScriptTokenizer.Keywords.Contains(word) &&
                        !JavaScriptTokenizer.ControlKeywords.Contains(word) &&
                        !JavaScriptTokenizer.TypeKeywords.Contains(word) &&
                        !JavaScriptTokenizer.LiteralKeywords.Contains(word) &&
                        seen.Add(word))
                    {
                        result.Add(new CompletionItem
                        {
    ...
    {
        var allKeywords = new HashSet<string>(JavaScriptTokenizer.Keywords);
        allKeywords.UnionWith(JavaScriptTokenizer.ControlKeywords);
        allKeywords.UnionWith(JavaScriptTokenizer.TypeKeywords);
        allKeywords.UnionWith(JavaScriptTokenizer.LiteralKeywords);
        ...
        foreach (var kw in allKeywords)
        {
            ...
            var kind = JavaScriptTokenizer.TypeKeywords.Contains(kw)
